...
Depends on the issue anatomy in [writing issues](issues.md); the adopting project's own `CONTRIBUTING.md` governs the content conventions it assumes.
...
| **Immediately before opening the change request** | the reviewer's route starts here | the state the change is landing into — see [pull requests](pull-requests.md) |
...
The tracking issue's first comment: `## Status — scope of work complete, three decisions open`.
...
| # | Item | Status | Where |
|---|---|---|---|
| 1 | Where the product sits in the wider process model | ✅ | `docs/strategy/positioning.md` v2, ADRs 0002–0005 |
| 2 | Benchmark and derive the feature set from evidence | ✅ | `docs/research/` (7 reports), `docs/product/capability-map.md` |
...
A fifth value is warranted only when there is a fifth answer to the two questions *and* it changes what the reader does — for example, where a deliverable is a record a human must confirm rather than a file that exists, *produced* and *confirmed* stop being the same state. Add a value by change request when the distinction is real, not before: a value nobody can apply yet dilutes the four that work.
...
Then, only for items where the row is not enough, a detail section numbered to match: `### 1 — Where the product sits`, `### 2 — What the benchmarks changed`. Numbering to match is what keeps a long comment navigable — that first comment is over 1,500 words and remains readable entirely because of this.
...
| No persistence, no review gate | ADR-0007 — a git-versioned workspace, a review queue, and a review command |
...
**Split open items by what happens if nobody acts, because that is what determines who has to read them.** Two outcomes exist, so there are two headings; a third heading is warranted only when ignoring an item produces a third outcome, and it is added by change request when it does.

Always split in two, under separate headings.
...
| Lives on afterwards as | a decision record, if it meets the bar | a roadmap phase, a backlog row, or a follow-up issue |
...
The tracking issue gets this right at the level of the heading itself: `## Open — three decisions, not outstanding work`. The heading does the work before the reader reaches the list.

Each open decision carries four things:

| Element | The tracking issue, item 1 |
|---|---|
| The choice | moving the published directory to a path without a space in it |
| The recommendation, with its reason | recommended — the published path currently contains a space |
| Why it was not executed | it touches a published path |
| The constraint that survives either outcome | *"The published package name must stay the same; it is immutable once anyone has installed it"* |

That fourth element is worth its own rule: **record the facts that make the decision cheap to revisit, so it does not have to be re-derived.** That issue's second comment does this explicitly — *"Two facts are recorded in the decision record so this can be revisited without re-deriving them."*

And when a decision is declined rather than taken:

**A decision declined without a revisit trigger is a decision deferred forever.** The second comment names the trigger: *"**Revisit when** packaging changes for a substantive reason: the next rung of the packaging ladder, a second distributed artifact, or CI tripping over the space."* Three named, observable conditions. Not "later".
...
The tracking issue's body implied the first business unit was the settled case and the second was the new, risky adopter. The status comment said otherwise, in the issue's own terms:

> One correction to the framing in this issue's premise, on the evidence: **the first business unit is the incumbent user of this artifact, not the future one.** The five skills were written and evaluated against its suppliers, the reference material is specific to its market, and the core record type holds that market's trade fields. The second unit is the new and harder adopter. The generalization risk runs the opposite way from expected.
...
The tracking issue reports three, and they are three different kinds. The kinds matter:

| Kind | The instance | Why it is easy to swallow |
|---|---|---|
| **Shipped guidance is wrong** | a market-conditional rung sits in the middle of the verification ladder with no condition attached — it produces nothing in a domestic market, so the ladder reports *unverified* where the truth is *invisible to a channel that does not apply here* | it works fine in the market it was written for |
| **The schema is wrong, found by external evidence** | `qualification.stage` was a single global enum; qualification is scoped and plural everywhere mature, and the governing quality standard requires approval status **and** scope of approval | nothing broke, because nothing implements it yet |
| **Your own reasoning was wrong** | *"one rule of mine was over-confident and is now corrected — scoring only self-attributed events is gameable by whoever fills the attribution field, so raw and attributed now publish side by side"* | **nothing forces it out** |

The third kind is the one this rule is really about. **Nobody else will find a defect in reasoning that exists only in your head.** A status update is the cheapest possible moment to surface it — before anything is built on it. The verification-ladder defect was caught by the leak test and cost a phase-0 fix; had it been caught after phase 1 it would have been a migration of real records.

Two further practices worth copying:

- **Say what found the defect.** *"Schema defect found by benchmarking"* is a heading that tells the reader which mechanism is working, so it gets used again. The decision index records the same thing about one record: *"the first decision written from external evidence rather than from internal reasoning… That is the record working as intended."*
- **Separate the fact from the defect when they differ.** *"The defect is not the fact; it is that a market-conditional channel sits in the core with no condition attached."* The claim is true where it applies. Naming precisely what is broken prevents an over-correction that deletes a good signal.

### 3. Report research or work that could not be completed, and why

Required by [research conventions](../docs/research.md): *"State what the pass could not reach. A benchmark that hides its gaps is worse than no benchmark, because conclusions get built on absence."*
...
The tracking issue's block, which is the model:

> - **Five of the platforms benchmarked publish no public product documentation.** Everything attributed to them is marketing copy and marked unverified. One of them carries the largest single unverified claim in the corpus.
> - Two more keep the relevant documentation behind login; two directories return HTTP 403.
> - One adjacent area was not benchmarked — out of scope per the positioning boundary.
...
`proposed` is not a soft `accepted` ([decision records](../docs/decisions.md)). **Never let a recommendation read as a decision taken.**
...
Both comments on the tracking issue are precise about this. The first: *"ADR-0009 is `proposed`, not accepted. Moving the published directory is recommended… but it touches a published path, so it was not executed."* The second: *"ADR-0009 moves from `proposed` to `accepted`, with both outcomes recorded in place rather than deferred to a follow-up."*

**The test before posting: for every recommendation in the comment, can a reader tell who decided it and when?** If not, mark it `proposed` in those words. And remember the comment is not the record — the decision record is. The comment reports that the record moved.
...
| Must also contain | — | the completion test and its result; the commits or change request that delivered it; what comes next, with a location |
...
2. **List the commits or the change request.** The tracking issue's second comment does exactly this, and the table is the format to copy: the docs-tree commit — *the product definition — 40 files, 7,014 insertions*; the second — *prototype removal and the two decisions above, recorded in the packaging decision record*.
...
4. **Rehome anything that survives the close, by name.** *"Nothing in this issue's scope of work is now outstanding. What comes next is build work rather than definition work, and it is sequenced in `docs/product/roadmap.md`."* A successor location, not a promise.
5. **State how the issue actually closes.** Commits carry `refs #N` in the subject, so merging does not auto-close — that comment says so and names the two options. See [pull requests](pull-requests.md).

**An update that says "nothing is outstanding" while leaving the issue open must say what it is waiting on.** The second comment is precisely this case: the scope is complete, the decisions are resolved, and the issue stays open pending the change request. Saying so is what keeps "open" meaningful.

---

## Worked examples

### Good — the two comments on the tracking issue
...
### 1 — Where the product sits                 <- per-item detail, numbered to match
### 2 — What the benchmarks changed
### 3 — One business unit, or all of them
### 4 — Roadmap
### 5 — Repository structure

## The problems in this issue, addressed       <- two-column table, problems verbatim
## Defect found in shipped guidance            <- honesty rule 2
...
the tools and the platforms, and the positioning doc is coming together
nicely. Still need to think about the platform question but I'm leaning towards
making it generic. Also the published folder has a space in the path which is
annoying, we should probably move it. Will pick this up again next week.
```

Every failure below maps to something the real comment got right.

| Line | Failure | What the real comment did |
|---|---|---|
...
| "mostly done", "most of the tools" | scope item 2 has no state and no location; "most" is unauditable | *"seven reports"*, listed, with the directory |
| "the positioning doc is coming together" | no path — a reader cannot check it | `docs/strategy/positioning.md` v2 |
| "leaning towards making it generic" | a recommendation written as a mood. It is scope item 3, the second-largest question in the issue, and it reads as neither `proposed` nor `accepted` | a decision record, with the nine-vs-nine evidence and an explicit verdict |
| "we should probably move it" | a decision meeting the bar for a record (packaging), buried mid-paragraph, with no options, no recommendation, no default, and no revisit trigger | a numbered heading at the end, with the immutable-name constraint recorded |
| *(absent)* | no not-reached block, though five of the platforms benchmarked publish no documentation at all — a gap that changes how the benchmark should be read | *"Research limits, recorded rather than hidden"*, ending in an impact assessment |
...
The comment is not short — it is 70 words that convey less than any one row of a real scope table.
...
For a closing comment, add the completion test and its result, a commit or change-request table, and the named successor location for anything that survives the close.
...
- [Research conventions](../docs/research.md) — *state what the pass could not reach*; *"unverified" is not "false"*. Honesty rule 3 extends these to issue comments.
- [Decision records](../docs/decisions.md) — *"`proposed` is not a soft `accepted`"*, the rule that accepted decisions are never rewritten, and the declined decision with a named revisit trigger quoted throughout.
- [Document lifecycle](../docs/document-lifecycle.md) — *merged ≠ built*, and the gap vs. out-of-scope distinction the not-reached block depends on.
...
- [Commit messages](../git/commit-messages.md) — the commit-body conventions that mirror these honesty rules.
- The adopting project's own `CONTRIBUTING.md` — where defects, roadmap phases and surviving work are rehomed at close.
